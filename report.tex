...
Given the same initial price and interest rate, the following figure(Figure. \ref{fig:1.stock_vs_vol})gives the histgram the the $\log(S_t)$ buy several different volatilities. 

\begin{figure}
	\centering
	%\begin{subfigure}
	%	
	%\end{subfigure}[t]{0.5\textwidth}
	\subfigure[$\sigma = 0.2$]{
		\includegraphics[scale=0.4]{./fig/{1.logST_hist_sigma0.2}.pdf}
	}

	\subfigure[$\sigma = 0.4$]{
		\includegraphics[scale=0.4]{./fig/{1.logST_hist_sigma0.4}.pdf}
	}

	\subfigure[$\sigma = 0.6$]{
		\includegraphics[scale=0.4]{./fig/{1.logST_hist_sigma0.6}.pdf}
	}

	\subfigure[$\sigma = 0.8$]{
		\includegraphics[scale=0.4]{./fig/{1.logST_hist_sigma0.8}.pdf}
	}
